...
export function PredictionOverlay({ close }: { close: () => void }) {
  ...
  const submit = () => {
    ...
    if (tooSmallInputPlayerIndex.length>0) {
        setError(`Zu kleine Schätzung für Spieler ${tooSmallInputPlayerIndex.map(i => players[i].name).join(', ')}`);
        return;
    }
    ...
    if (tooLargeInputPlayerIndex.length>0) {
      setError(`Zu große Schätzung für Spieler ${tooLargeInputPlayerIndex.map(i => players[i].name).join(', ')}`);
      return;
    }
      ...
      <div className="bg-white p-4 rounded shadow w-96">
        ...
        {players.map((p, i) => (
          <div key={i} className="flex flex-col xs:flex-row justify-between mb-2">
            <div>
              <span>{p.name}</span>{p.points.at(-1) && (<span className="italic">&nbsp;({p.points.at(-1)} Punkte)</span>)}
            </div>
            <input
              type="number"
              min={0}
