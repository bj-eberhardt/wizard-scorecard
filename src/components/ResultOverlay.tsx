...
export function ResultOverlay({ close }: { close: () => void }) {
  ...
  const submit = () => {
    ...
    if (tooSmallInputPlayerIndex.length>0) {
      setError(`Der Wert ist zu klein für Spieler ${tooSmallInputPlayerIndex.map(i => players[i].name).join(', ')}`);
      return;
    }
    ...
    if (tooLargeInputPlayerIndex.length>0) {
      setError(`Der Wert ist zu groß für Spieler ${tooLargeInputPlayerIndex.map(i => players[i].name).join(', ')}`);
      return;
    }

    for (let i = 0; i < players.length; i++) {
      if (useAnniversaryRules && wolkeFlags[i] && results[i] < 1) {
        setError(`Spieler "${players[i].name}" hat die Wolke bekommen und muss mindestens 1 Stich haben.`);
        return;
      }
    }
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-60 flex items-center justify-center z-20">
      <div className="bg-white p-4 rounded shadow w-96">
        <h2 className="text-lg font-bold mb-2">Spiele Runde {currentRound} und trage dann die Ergebnisse ein:</h2>
        {players.map((p, i) => (
          <div key={i} className="flex justify-between mb-2">
            <div><span>{p.name}</span><span className="italic">&nbsp;(Tipp: {p.predictions[currentRound - 1]})</span></div>
            <div><input
              type="number"
              min={0}
              max={currentRound}
