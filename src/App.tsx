...
export default function App() {
  const { players, setPlayerNames, gameStarted, currentRound, totalRounds, overlay, setOverlay, setUseAnniversaryRules, resetGame } = useGameStore();
  ...
  return (
        ...
        <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-50">
          <div className="bg-white p-6 rounded shadow-lg text-center">
            ...
            <div className="mt-4 flex justify-center gap-4">
              <button onClick={handleNewGame} className="bg-red-600 text-white px-4 py-2 rounded">Ja, beenden</button>
              <button onClick={() => setShowConfirm(false)} className="bg-gray-300 px-4 py-2 rounded">Abbrechen</button>
            </div>
          </div>
        </div>
      )}
    ...
    {isGameOver && (
        <div className="mt-2">
            <button onClick={() => handleNewGame()} className="bg-red-500 text-white px-4 py-2 rounded">
                Neues Spiel starten
            </button>
        </div>
    )}
